...
export async function getWorkerToken(options: WorkerAuthOptions): Promise<string> {
  ...
  const prefix = options.logPrefix ? `[${options.logPrefix}]` : '[WorkerAuth]'

  if (IS_DEV) console.log(`${prefix} Authenticating with worker...`)
  ...
  if (IS_DEV) console.log(`${prefix} Authenticated successfully`)

  return token
}

export function clearWorkerAuthCache(): void {
  tokenCache.clear()
}
