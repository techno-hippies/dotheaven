...
export const MusicPage: Component = () => {
  ...
  const sharedEntries = createMemo(() => sharedContent() ?? [])
  const sharedCount = createMemo(() => sharedEntries().length)
  const [sharedLastSeenAt, setSharedLastSeenAt] = createSignal(0)

  createEffect(() => {
    const addr = auth.pkpInfo()?.ethAddress
    if (!addr) {
      setSharedLastSeenAt(0)
      return
    }
    const raw = localStorage.getItem(sharedSeenStorageKey(addr))
    const seenAt = raw ? parseInt(raw, 10) : 0
    setSharedLastSeenAt(Number.isFinite(seenAt) && seenAt > 0 ? seenAt : 0)
  })

  const sharedNewCount = createMemo(() => {
    const seenAt = sharedLastSeenAt()
    if (seenAt <= 0) return sharedCount()
    return sharedEntries().filter((e) => (e.grantedAt || e.uploadedAt || 0) > seenAt).length
  })

  const markSharedSeen = () => {
    const addr = auth.pkpInfo()?.ethAddress
    if (!addr) return
    const nowSec = Math.floor(Date.now() / 1000)
    localStorage.setItem(sharedSeenStorageKey(addr), String(nowSec))
    setSharedLastSeenAt(nowSec)
  }
    ...
    <div class="h-full overflow-y-auto">
      ...
      <div class="flex flex-col pb-2 md:hidden -mx-4">
        ...
        <LibraryEntryCard
          ...
          badge={sharedNewCount() > 0 ? `${sharedNewCount()} new` : undefined}
