...
export const ProfileHeader: Component<ProfileHeaderProps> = (props) => {
  ...
  return (
    <div class={cn('relative', props.class)}>
      ...
      <div class="max-w-4xl mx-auto px-4 md:px-8 pb-6">
        ...
        {/* Follower / Following counts — inline under handle */}
        <Show when={props.followerCount !== undefined || props.followingCount !== undefined}>
          <div class="flex items-center gap-1 text-base mb-3">
            <Show when={props.followerCount !== undefined}>
              <button
                type="button"
                class="hover:underline text-[var(--text-secondary)]"
                onClick={() => props.onFollowerCountClick?.()}
              >
                <span class="font-semibold text-[var(--text-primary)]">{props.followerCount ?? 0}</span> followers
              </button>
            </Show>
            <Show when={props.followerCount !== undefined && props.followingCount !== undefined}>
            ...
            <Show when={props.followingCount !== undefined}>
              ...
              >
                <span class="font-semibold text-[var(--text-primary)]">{props.followingCount ?? 0}</span> following
              </button>
            </Show>
          </div>
        </Show>

        {/* Bio — clamped to 2 lines with "More" toggle */}
        <Show when={props.bio}>
          <div class="mb-3 max-w-[700px]">
            <p
              class={cn(
                'text-base text-[var(--text-primary)] whitespace-pre-wrap',
                !bioExpanded() && 'line-clamp-2',
              )}
            >
              {props.bio}
            </p>
            {/* Show "More" only if text is long enough to be clamped */}
            <Show when={props.bio && props.bio.length > 120 && !bioExpanded()}>
              <button
                type="button"
                class="text-base text-[var(--text-muted)] hover:text-[var(--text-primary)] mt-0.5"
        ...
        {/* Links row */}
        <Show when={props.url || props.twitter || props.github || props.telegram}>
