            ...
            .when(self.selected_type == RoomType::DjSet, |el| {
                el.child(
                    div()
                        ...
                        .child("Solo room. Use the broadcast page to share app/system audio (or use mic)."),
                )
            })
            .when(self.selected_type == RoomType::Duet, |el| {
                el.child(
                    div()
                        .text_sm()
                        .text_color(theme.muted_foreground)
                        .child("Payout: for now, ticket revenue routes to your wallet (splits coming later)."),
                )
            })
            .when(self.selected_type == RoomType::Duet, |el| {
