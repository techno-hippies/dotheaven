# AGENTS.md — `apps/gpui-poc`

This document is for coding agents working in the GPUI PoC.

## Scope
- Applies to everything under `apps/gpui-poc/`.
...
```bash
cargo fmt --manifest-path apps/gpui-poc/Cargo.toml
CARGO_TARGET_DIR=/tmp/gpui-poc-check-codex cargo check --manifest-path apps/gpui-poc/Cargo.toml
apps/gpui-poc/scripts/check_rs_size.sh 450
```

## Size Guardrail
- Rust file cap is `450` lines unless explicitly allowlisted in `apps/gpui-poc/.rs-size-allowlist`.
