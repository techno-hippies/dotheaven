/**
 * Heaven Name Registry - Tempo Moderato
 *
...
import { REGISTRY_V1, RECORDS_V1, HEAVEN_NODE, PIRATE_NODE } from '@heaven/core'
import { tempoModerato } from '../chains'
...
function tldForParentNode(parentNode: `0x${string}`): string | null {
  const normalized = parentNode.toLowerCase()
  if (normalized === HEAVEN_NODE.toLowerCase()) return 'heaven'
  if (normalized === PIRATE_NODE.toLowerCase()) return 'pirate'
  return null
}
 ...
 *
 * The sponsor PKP pays gas on Tempo. User signs an EIP-191 message
...
export async function getPrimaryNamesBatch(addresses: `0x${string}`[]): Promise<PrimaryNameResult[]> {
  ...
  return results.map((result, i) => {
    ...
    if (result.status === 'failure' || !result.result) {
      return { address: addr, label: null, node: null, parentNode: null, fullName: null }
    }
    ...
    const labelHash = keccak256(toBytes(label))
    const node = keccak256(encodePacked(['bytes32', 'bytes32'], [parentNode, labelHash]))
    const tld = tldForParentNode(parentNode)
    const fullName = tld ? `${label}.${tld}` : label
    return { address: addr, label, node, parentNode, fullName }
  })
}
...
export { REGISTRY_V1, RECORDS_V1, HEAVEN_NODE, PIRATE_NODE }
