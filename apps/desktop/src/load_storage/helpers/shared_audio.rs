...
#[derive(Debug, Clone)]
pub(crate) struct ParsedContentHeaderPrefix {
    pub(crate) lit_ciphertext_base64: String,
    pub(crate) data_to_encrypt_hash_hex: String,
}

/// Parse only the (ciphertext, dataToEncryptHash) portion of a content blob header.
///
/// This is useful when callers fetch only a small prefix via HTTP range requests.
pub(crate) fn parse_content_header_prefix(
    blob: &[u8],
) -> Result<ParsedContentHeaderPrefix, String> {
    match parse_content_header_prefix_raw(blob) {
        Ok(parsed) => Ok(parsed),
        Err(raw_err) => {
            let item = DataItem::from_bytes(blob)
                .map_err(|_| format!("Failed parsing content header: {raw_err}"))?;
            parse_content_header_prefix_raw(&item.data)
                .map_err(|inner| format!("Failed parsing content header dataitem payload: {inner}"))
        }
    }
}

pub(crate) fn parse_content_header_prefix_raw(
    blob: &[u8],
) -> Result<ParsedContentHeaderPrefix, String> {
    fn take<'a>(
        blob: &'a [u8],
