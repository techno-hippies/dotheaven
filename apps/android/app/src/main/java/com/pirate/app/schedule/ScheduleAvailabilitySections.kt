...
internal fun AvailabilityFilterBar(
  ...
  Row(
    ...
    OutlinedButton(onClick = onOpenFilter) { Text("Filter · $filterLabel") }
  }
}
    ...
    Surface(
      ...
      Column(
        ...
        Row(
          ...
          SlotApplyScope.entries.forEach { option ->
            if (option == selectedScope) {
              Button(onClick = { onScopeChange(option) }, enabled = !busy) { Text(option.label) }
            } else {
              OutlinedButton(onClick = { onScopeChange(option) }, enabled = !busy) { Text(option.label) }
            }
          }
        }
          ...
          Row(
            ...
            SlotApplyRange.entries.forEach { option ->
              if (option == selectedRange) {
                Button(onClick = { onRangeChange(option) }, enabled = !busy) { Text(option.label) }
              } else {
                OutlinedButton(onClick = { onRangeChange(option) }, enabled = !busy) { Text(option.label) }
              }
            }
          }
        }
        ...
        Button(
          ...
          onClick = onApply,
          enabled = !busy,
        ) { Text(if (busy) "Applying..." else "Apply Changes") }
      }
    }
  }
}
