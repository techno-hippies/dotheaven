...
object ContentKeyManager {
    ...
    private val HEX_REGEX = Regex("^(0x)?[0-9a-fA-F]{1,64}$")
    ...
    fun load(context: Context): ContentKeyPair? {
        ...
        val privHex = if (HEX_REGEX.matches(storedPriv.trim())) {
            // One-time migration from legacy plaintext key material.
            val normalized = storedPriv.trim().removePrefix("0x").removePrefix("0X").lowercase()
            prefs.edit().putString("private_key", encryptPrivateKeyHex(normalized)).apply()
            normalized
        } else {
            decryptPrivateKeyHex(storedPriv)
        }
