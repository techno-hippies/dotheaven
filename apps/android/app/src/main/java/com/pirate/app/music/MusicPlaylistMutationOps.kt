...
private val PLAYLIST_MUTATION_ADDRESS_REGEX = Regex("^0x[a-fA-F0-9]{40}$")
...
internal suspend fun sharePlaylistToWalletWithUi(
  context: Context,
  hostActivity: androidx.fragment.app.FragmentActivity?,
  playlist: PlaylistDisplayItem,
  ...
  val playlistId = playlist.id.trim()
  if (playlist.isLocal || !playlistId.startsWith("0x", ignoreCase = true)) {
    onShowMessage("Playlist sharing is only available for on-chain playlists")
    return false
  }

  val owner = ownerEthAddress?.trim()?.lowercase().orEmpty()
  if (!isAuthenticated || owner.isBlank() || tempoAccount == null) {
    onShowMessage("Sign in with Tempo passkey to share playlists")
    return false
  }

  val normalizedRecipientInput = recipientInput.trim().removePrefix("@").lowercase()
  if (normalizedRecipientInput.isBlank()) {
    onShowMessage("Enter recipient wallet address, .heaven, or .pirate name")
    return false
  }
  if (
    normalizedRecipientInput.startsWith("0x") &&
    !PLAYLIST_MUTATION_ADDRESS_REGEX.matches(normalizedRecipientInput)
  ) {
    onShowMessage("Wallet address must be 0x followed by 40 hex characters")
    return false
  }
  val recipientAddress =
    if (PLAYLIST_MUTATION_ADDRESS_REGEX.matches(normalizedRecipientInput)) {
      normalizedRecipientInput
    } else {
      val tld = normalizedRecipientInput.substringAfterLast('.', missingDelimiterValue = "")
      if (
        normalizedRecipientInput.contains('.') &&
        tld.isNotBlank() &&
        tld != "heaven" &&
        tld != "pirate"
      ) {
        onShowMessage("Recipient name must end with .heaven or .pirate")
        return false
      }

      val resolution =
        runCatching {
          TempoNameRecordsApi.resolveAddressForName(normalizedRecipientInput)
            ?.trim()
            ?.lowercase()
        }
      val resolvedAddress = resolution.getOrNull()
      if (!resolvedAddress.isNullOrBlank()) {
        resolvedAddress
      } else {
        if (resolution.exceptionOrNull() != null) {
          onShowMessage("Name lookup failed. Check connection and try again.")
        } else {
          onShowMessage("No wallet found for \"$normalizedRecipientInput\"")
        }
        return false
      }
    }
  if (recipientAddress.equals(owner, ignoreCase = true)) {
    onShowMessage("Cannot share playlist to your own wallet")
    return false
  }
  ...
  val sessionKey =
    resolvePlaylistMutationSessionKey(
      ...
      tempoAccount = tempoAccount,
      owner = owner,
      failureMessage = "Session expired. Sign in again to share playlists.",
      onShowMessage = onShowMessage,
    ) ?: return false

  val result =
    TempoPlaylistApi.sharePlaylist(
      account = tempoAccount,
      sessionKey = sessionKey,
      playlistId = playlistId,
  ...
  if (!result.success) {
    onShowMessage("Playlist share failed: ${result.error ?: "unknown error"}")
    return false
  }

  onShowMessage("Shared playlist with ${shortAddress(recipientAddress, minLengthToShorten = 10)}")
  return true
}
