...
object LitRust {
  ...
  data class FetchAndDecryptContentResult(
  ...
  fun fetchAndDecryptContentRaw(network: String, rpcUrl: String, paramsJson: String): String {
    return jni.fetchAndDecryptContent(network.trim(), rpcUrl.trim(), paramsJson)
  }

  fun fetchAndDecryptContent(
    network: String,
    rpcUrl: String,
    paramsJson: String,
  ): FetchAndDecryptContentResult {
    val result = unwrapEnvelope(fetchAndDecryptContentRaw(network, rpcUrl, paramsJson))
    val audioBase64 = result.optString("audioBase64", "").trim()
    if (audioBase64.isEmpty()) throw IllegalStateException("Decrypt succeeded but returned no audioBase64")
    val bytes = result.optInt("bytes", 0)
    val mimeType = result.optString("mimeType", "").trim().ifBlank { null }
    val sourceUrl = result.optString("sourceUrl", "").trim()
    if (sourceUrl.isEmpty()) throw IllegalStateException("Decrypt succeeded but returned no sourceUrl")
    return FetchAndDecryptContentResult(
