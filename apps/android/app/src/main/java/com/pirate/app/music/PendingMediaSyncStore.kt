...
object PendingMediaSyncStore {
  ...
  suspend fun upsertRefs(
    ...
    withContext(Dispatchers.IO) {
      ...
      val normalizedLyrics = normalizeNullableRef(lyricsRef)
      ...
      val merged =
        PendingTrackMediaEntry(
          ...
          lyricsRef = normalizedLyrics ?: existing?.lyricsRef,
          updatedAtMs = updatedAtMs,
        )
      val finalized =
        if (merged.coverRef.isNullOrBlank() && merged.lyricsRef.isNullOrBlank()) {
          null
        } else {
          merged
        }
  ...
  suspend fun markCoverSynced(
    ...
    withContext(Dispatchers.IO) {
      ...
      val finalized = if (updated.lyricsRef.isNullOrBlank()) null else updated
  ...
  private fun loadInternal(context: Context): List<PendingTrackMediaEntry> {
    ...
    return runCatching {
      ...
      for (i in 0 until arr.length()) {
        ...
        val lyricsRef = normalizeNullableRef(obj.optString("lyricsRef", ""))
        if (coverRef == null && lyricsRef == null) continue
    ...
    for (entry in entries) {
      arr.put(
        JSONObject()
          ...
          .put("lyricsRef", entry.lyricsRef ?: JSONObject.NULL)
