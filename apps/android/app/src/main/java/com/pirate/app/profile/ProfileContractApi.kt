...
object ProfileContractApi {
  ...
  private fun fetchProfileFromSubgraph(addr: String): ContractProfileData? {
    ...
    val json = runCatching { postProfileQuery(query) }.getOrNull() ?: return null
    ...
    return ContractProfileData(
      ...
      nameHash = normalizeProfileHash(profile.optString("nameHash", "")),
      age = profile.optInt("age", 0),
      heightCm = profile.optInt("heightCm", 0),
      nationality = profileBytes2ToCode(profile.optString("nationality", "")) ?: "",
      languages = unpackProfileLanguages(profile.optString("languagesPacked", "0")),
      friendsOpenToMask = profile.optInt("friendsOpenToMask", 0),
      locationCityId = normalizeProfileHash(profile.optString("locationCityId", "")),
      locationLatE6 = profile.optInt("locationLatE6", 0),
      locationLngE6 = profile.optInt("locationLngE6", 0),
      schoolId = normalizeProfileHash(profile.optString("schoolId", "")),
      skillsCommit = unpackProfileTagIds(profile.optString("skillsCommit", "")).joinToString(","),
      hobbiesCommit = unpackProfileTagIds(profile.optString("hobbiesCommit", "")).joinToString(","),
