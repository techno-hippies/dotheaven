...
object LocalSecp256k1Store {
  ...
  private fun getOrCreateKeyPair(context: Context, userAddress: String): ECKeyPair {
    ...
    if (!stored.isNullOrBlank()) {
      val privateKeyHex = if (HEX_REGEX.matches(stored.trim())) {
        // One-time migration from legacy plaintext keys.
        val normalized = stored.trim().removePrefix("0x").removePrefix("0X").lowercase()
        prefs.edit().putString(storageKey, encryptPrivateKeyHex(normalized)).apply()
        normalized
      } else {
        decryptPrivateKeyHex(stored)
      }
