 ...
 *
 * Usage:
 *   bun src/smoke-test-duet-cdp.ts
 *
 * Required env:
 ...
 *   - X402_FACILITATOR_BASE_URL is configured on the worker
 *   - X402_FACILITATOR_AUTH_TOKEN only if your facilitator requires it (Coinbase CDP does; OpenX402 does not)
 */
...
function readPaymentResponse(headers: Headers, label: string): any {
  ...
  assert(decoded?.facilitator === 'cdp', `${label}: facilitator is cdp`)
  return decoded
}
...
async function main() {
  const replaySourceUrl = 'data:text/plain;base64,ZHVldC1yZXBsYXktY2RwLXNhbXBsZQ=='
  ...
  console.log('\n═══ Duet Facilitator x402 Smoke Test ═══')
  ...
  const recordingComplete = await api(
    ...
    undefined,
    {
      load_dataitem_id: 'demo-dataitem-cdp-123',
  ...
  console.log('\n═══ Duet CDP x402 smoke test passed ═══\n')
}
