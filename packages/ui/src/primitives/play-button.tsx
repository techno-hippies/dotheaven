...
export const PlayButton: Component<PlayButtonProps> = (props) => {
  ...
  return (
    ...
    >
      {local.isPlaying ? (
        // Pause icon
        <svg class={cn(local.size === 'lg' ? 'w-6 h-6' : 'w-4 h-4')} fill="currentColor" viewBox="0 0 24 24">
          <path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z" />
        </svg>
      ) : (
        // Play icon
        <svg class={cn(local.size === 'lg' ? 'w-6 h-6' : 'w-4 h-4', 'ml-0.5')} fill="currentColor" viewBox="0 0 24 24">
