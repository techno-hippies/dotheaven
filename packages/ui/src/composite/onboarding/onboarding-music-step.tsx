...
export const OnboardingMusicStep: Component<OnboardingMusicStepProps> = (props) => {
  const [mode, setMode] = createSignal<Mode>('prompt')
    ...
    <div class={cn('flex flex-col gap-5 w-full', props.class)}>
      ...
      <Show when={mode() === 'prompt'}>
        <div class="flex flex-col gap-4">
          ...
          <button
            type="button"
            class="text-sm text-[var(--text-muted)] hover:text-[var(--text-secondary)] transition-colors text-center"
        ...
        <For each={props.artists}>
          {(artist) => (
            <button
              type="button"
              class={cn(
                'relative flex flex-col items-center gap-2 p-3 rounded-md transition-all duration-150',
