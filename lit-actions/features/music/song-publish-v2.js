 ...
 * Song Publish v2
 *
 * Storage-agnostic evolution of song-publish-v1.
 * Backward compatible with v1 params while adding:
 * - ref aliases: audioRef/coverRef/instrumentalRef/vocalsRef/canvasRef
 * - storage key aliases: storageEncryptedKey/storagePlaintextKey
 *
 * Notes:
 * - This version still uploads output artifacts via Filebase/IPFS.
 * - Input refs can be https://, ipfs://, bare CID (Qm/bafy), or ar://.
 * - ar:// inputs are resolved via https://arweave.net/<id> for fetch.
 */
...
async function fetchAndValidate(urlOrInline, maxBytes, allowedTypes) {
  ...
  const url = normalizeFetchUrl(urlOrInline);
...
const main = async () => {
  try {
    ...
    const audioInput = audioRef ?? audioUrl;
    const coverInput = coverRef ?? coverUrl;
    const instrumentalInput = instrumentalRef ?? instrumentalUrl;
    const vocalsInput = vocalsRef ?? vocalsUrl;
    const canvasInput = canvasRef ?? canvasUrl;

    must(audioInput, "audioUrl or audioRef");
    must(coverInput, "coverUrl or coverRef");
    must(instrumentalInput, "instrumentalUrl or instrumentalRef");
    must(vocalsInput, "vocalsUrl or vocalsRef");
    ...
    // ========================================
    const audio = await fetchAndValidate(audioInput, 50 * 1024 * 1024, ALLOWED_AUDIO_TYPES);
    const cover = await fetchAndValidate(coverInput, 5 * 1024 * 1024, ALLOWED_IMAGE_TYPES);
    const instrumental = await fetchAndValidate(instrumentalInput, 50 * 1024 * 1024, ALLOWED_AUDIO_TYPES);
    const vocals = await fetchAndValidate(vocalsInput, 50 * 1024 * 1024, ALLOWED_AUDIO_TYPES);
    const canvas = canvasInput ? await fetchAndValidate(canvasInput, 30 * 1024 * 1024, ALLOWED_VIDEO_TYPES) : null;
    ...
    const effectiveStorageMode = String(storageMode || "filebase").toLowerCase();
    const _existingRefPolicy = String(existingRefPolicy || "reuse");
    if (effectiveStorageMode !== "filebase") {
      throw new Error(`Unsupported storageMode for song-publish-v2: ${effectiveStorageMode} (expected "filebase")`);
    }

    const filebaseKey = await decryptKey(storageEncryptedKey ?? filebaseEncryptedKey, storagePlaintextKey ?? filebasePlaintextKey);
    if (!filebaseKey) {
      throw new Error("storageEncryptedKey/storagePlaintextKey or filebaseEncryptedKey/filebasePlaintextKey is required");
    }
    ...
    const resultJson = await Lit.Actions.runOnce(
      { waitForResponse: true, name: "songPublishV2" },
      async () => {
        try {
