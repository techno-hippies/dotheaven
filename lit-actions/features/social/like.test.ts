#!/usr/bin/env bun
/**
 * Test Like v1 Lit Action
 *
 * Verifies:
 *  - EIP-191 signature verification
 *  - Sponsor PKP broadcasts likeFor()/unlikeFor() on MegaETH
 *  - Returns txHash + action confirmation
 *
 * Usage:
 *   bun features/social/like.test.ts                  # Full test (broadcast like)
 *   bun features/social/like.test.ts --unlike         # Test unlike
 *   bun features/social/like.test.ts --dry-run        # Verify sig only, no broadcast
 *
 * Prerequisites:
 *   - Deploy action: bun scripts/setup.ts likeV1
 *   - PRIVATE_KEY env var
 */
...
const doUnlike = args.includes("--unlike");

async function main() {
  const action = doUnlike ? "unlike" : "like";

  console.log("Test Like v1");
  ...
  console.log(
    `   Action CID:  ${Env.cids.likeV1 || "(not deployed)"}`
  );

  if (!Env.cids.likeV1) {
    console.error(
      "\nNo likeV1 action CID found. Run setup.ts first:"
    );
    console.error("   bun scripts/setup.ts likeV1");
    process.exit(1);
  }
  ...
  const postId = "0x" + ethers.keccak256(ethers.toUtf8Bytes("heaven:test-like-post")).slice(2);
