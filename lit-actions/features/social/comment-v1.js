 ...
 * Comment v1
 *
 * Add a comment to a post on-chain via EngagementV2 on MegaETH.
 * Sponsor PKP pays gas — user is gasless.
 *
 * Flow:
 * 1. Verify EIP-191 signature over postId + text hash
 * 2. Sponsor PKP signs + broadcasts commentFor() on MegaETH
 * 3. Return txHash + commentId
 *
 * Required jsParams:
 ...
 *
 * Returns: { success, version, user, postId, text, txHash }
 */
...
const ENGAGEMENT_V2_ABI = [
  "function commentFor(address author, bytes32 postId, string text) external returns (uint256)",
];
...
const main = async () => {
  try {
    ...
    if (text.length > MAX_COMMENT_LENGTH) {
      throw new Error(`Comment text exceeds ${MAX_COMMENT_LENGTH} character limit`);
    }
    ...
    const calldata = iface.encodeFunctionData("commentFor", [
