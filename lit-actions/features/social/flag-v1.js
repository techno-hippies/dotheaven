 ...
 * Flag v1
 *
 * Flag a post for moderation on-chain via EngagementV2 on MegaETH.
 * Sponsor PKP pays gas — user is gasless.
 * Idempotent: flagging an already-flagged post is a no-op.
 *
 * Flow:
 * 1. Verify EIP-191 signature over postId + reason
 * 2. Sponsor PKP signs + broadcasts flagFor() on MegaETH
 * 3. Return txHash
 *
 * Required jsParams:
 * - userPkpPublicKey: User's PKP public key
 * - postId: bytes32 post identifier
 * - reason: uint8 flag reason code (0=spam, 1=abuse, 2=nsfw, 3=other)
 ...
 *
 * Returns: { success, version, user, postId, reason, txHash }
 */
...
const ENGAGEMENT_V2_ABI = [
  "function flagFor(address flagger, bytes32 postId, uint8 reason) external",
];
...
const main = async () => {
  try {
    ...
    // reason can be 0, so check for undefined/null explicitly
    if (reason === undefined || reason === null) {
      throw new Error("reason is required");
    }
    const reasonNum = Number(reason);
    if (!Number.isInteger(reasonNum) || reasonNum < 0 || reasonNum > 255) {
      throw new Error("reason must be a uint8 (0-255)");
    }
    ...
    // ========================================
    const message = `heaven:flag:${postId}:${reasonNum}:${timestamp}:${nonce}`;
    ...
    const calldata = iface.encodeFunctionData("flagFor", [
