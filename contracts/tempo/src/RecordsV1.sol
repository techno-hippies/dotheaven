...
contract RecordsV1 is Ownable {
    ...
    function _hashMessage(string memory message) internal view returns (bytes32) {
        // Include chainId and contract address to prevent cross-chain replay
        string memory bound = string.concat(
            message,
            ":",
        ...
        return keccak256(
            abi.encodePacked(
                ...
                _uintToString(bytes(bound).length),
                bound
            )
        );
    }
