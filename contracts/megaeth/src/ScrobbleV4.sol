...
contract ScrobbleV4 is AccountBinding {
    ...
    /// @param factory_ HeavenAccountFactory address (immutable, security-critical)
    /// @param operator_ Initial operator address (for global track ops)
    constructor(address factory_, address operator_) AccountBinding(factory_) {
    ...
    // ── User-facing: register + scrobble (AA-gated) ─────────────────────

    /// @notice Register (optional) + scrobble in one tx.
    ///         msg.sender must be user's SimpleAccount (factory-verified).
    ...
    // ── User-facing: scrobble only (AA-gated) ────────────────────────────

    /// @notice Scrobble existing tracks. msg.sender must be user's SimpleAccount.
